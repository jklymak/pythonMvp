...
def bin1ddata(x, y, xi, ppbin=False, binval='median'):
    """
    ...
    #nrow = yi.shape[0]
    ncol = xi.shape[0]
    grid = np.empty((1,ncol), dtype=xi.dtype)
    ...
    for col in xrange(ncol):
        ...
        if npts > 0:
            if median:
                grid[row,0] = np.median(y[ind])
            else:
                grid[row,0] = np.mean(y[ind])
            if ppbin: bins[row,0] = npts
        else:
            grid[row,0] = np.nan
            if ppbin: bins[row,0] = 0
